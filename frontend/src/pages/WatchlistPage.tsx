 ...
 *
 * Sprint 4: watchlist entries now come from the database via useWatchlist.
 * Each WatchlistEntry already includes the related Movie object (joined by
 * Prisma in watchlistRepo.getAll()), so no separate movie fetch is needed.
 */
...
const WatchlistPage = () => {
  ...
  if (loading) {
    return <main className="page-main"><p>Loading watchlist…</p></main>;
  }
                ...
                {unwatched.map((entry) => (
                  <div key={entry.movieId} className="watchlist-movie-wrap">
                    ...
                    <button
                      ...
                      onClick={() => toggleWatched(entry.movieId)}
                    >
                      ✓ Mark as Watched
                    </button>
                  </div>
